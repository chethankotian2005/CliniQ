...
class TwilioService:
    ...
    def sanitize_sms_message(self, message):
        """Clean message for SMS delivery to avoid Unicode/emoji issues"""
        # Remove problematic characters that might cause delivery failures
        import re
        ...
        # Ensure message is not too long (SMS limit is 1600 chars, but 160 is safer)
        if len(message) > 160:
            message = message[:157] + "..."
    ...
    def send_sms(self, to_number, message, notification_id=None):
        ...
        try:
            # Sanitize message for better delivery
            clean_message = self.sanitize_sms_message(message)
